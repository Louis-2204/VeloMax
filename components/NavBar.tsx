...
export function Navbar({ user }: { user: any }) {
    ...
    useEffect(() => {
        ...
        mainDiv && mainDiv.addEventListener("scroll", () => {
    ...
    return pahtname !== "/login" ? (
        <div className={`flex fixed w-full py-4 px-16 top-0 z-50 ${background} justify-center transition-colors duration-500`}>
            <div className="w-full max-w-8xl flex divide-x-4 divide-[#505050]">
                <div className="flex items-center justify-center px-4">
                    <h1 className="text-4xl font-bold text-vm_gray transition-colors duration-500">
                        VeloMax
                    </h1>
                </div>
                ...
                <NavigationMenu className="max-w-full flex items-center justify-between w-full">
                    ...
                    <NavigationMenuList>
                        <NavigationMenuItem className="px-1 cursor-pointer" onClick={() => { ToggleTheme() }}>
                            {theme === "dark" ? (
