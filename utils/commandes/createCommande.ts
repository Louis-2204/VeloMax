...
export async function createCommande(infos: {
  items: { id: string; nom: string; quantite: number; prix: number; image: string; type: 'vélo' | 'pièce' }[];
  nom: string;
  prenom: string;
  adresse: string;
  ville: string;
  ...
  if (infos.items.some((item) => item.type === 'vélo')) {
    const { data: dataCommandeVelo, error: errorCommandeVelo } = await supabase.from('commandes_velos').insert(
      infos.items.map((item) => ({
        id_velo: item.id,
        id_commande: dataCommande.id_commande,
        quantite: item.quantite,
