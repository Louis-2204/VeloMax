    ...
    <div className="w-full max-w-8xl h-full flex-col">
      <div className="w-full flex justify-center pb-10 px-6">
        <div className="flex w-full max-w-lg py-5 items-center text-black/75">
          <span
            className={`relative flex justify-center items-center flex-shrink h-12 w-12 rounded-full ${
              state === 'récapitulatif' ? 'border-2 border-vm_secondary' : ''
            } ${
          ...
          >
            ...
            <div
              className={`absolute flex gap-1 items-center -bottom-6 text-sm ${
          ...
          <span
            className={`relative flex justify-center items-center flex-shrink h-12 w-12 rounded-full ${
              state === 'livraison' ? 'border-2 border-vm_secondary' : ''
            } ${state === 'paiement' || state === 'done' ? 'bg-vm_secondary' : 'bg-vm_bg_gray'}`}
          >
            <div className="font-bold text-lg">2</div>
            <div
              className={`absolute flex gap-1 items-center -bottom-6 text-sm ${
                state === 'paiement' || state === 'done' ? 'text-vm_secondary' : 'text-vm_text_gray_secondary'
              }`}
            >
              Livraison{' '}
          ...
          <div
            className={`flex-grow border-t-2 ${
              state === 'paiement' || state === 'done' ? 'border-vm_secondary' : 'border-vm_bg_gray'
            }`}
          ></div>
          <span
            className={`relative flex justify-center items-center flex-shrink h-12 w-12 rounded-full ${
              state === 'paiement' ? 'border-2 border-vm_secondary' : ''
            } ${state === 'done' ? 'bg-vm_secondary' : 'bg-vm_bg_gray'}`}
          >
            <div className="font-bold text-lg">3</div>
            <div
              className={`absolute flex gap-1 items-center -bottom-6 text-sm ${
                state === 'done' ? 'text-vm_secondary' : 'text-vm_text_gray_secondary'
              }`}
            >
      ...
      {(state === 'paiement' || state === 'done') && (
        <Paiement
          ...
          pay={() => {
            setState('done'),
              alert(
                "Paiement effectué avec succès, vous serrez redirigé automatiquement vers la page d'accueil dans quelques secondes."
              ),
              setTimeout(() => {
